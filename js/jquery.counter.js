/*
   ...
   Minor Modifications (to fit needs) by Sawyer Altman
    ...
    $.fn.extend({
        counter: function(options) {
            ...
            var defaults = {
                ...
                goal: 200       // count {to || from} this number                
            };
            ...
            return this.each(function() {
				...
				$('#charRemaining').html("<span>" + get_msg_equation($($obj).val().length ) + "</span>" + msg);

                // Cache the counter selector
                var $currentCount = $("#charRemaining span");
                        ...
                        if (flag) {
                            ...
                            if ((event.keyCode !== 46 && event.keyCode !== 8)) {
                                if ($(this).val().length > options.goal && options.type === 'char') {
									...
									for (var i = 0; i < 2; i++) {
										$('label#charRemaining').stop().animate({ 'color' : 'red', 'text-shadow' : '0 0 5px darkred' }, 200);
										setTimeout("$('label#charRemaining').stop().animate({ 'color' : 'grey', 'text-shadow' : 'none' }, 200)", 200)
									}
